...
type LectureGroup = Lecture[];
type CartType = {
  lectureGroups: LectureGroup[] | null; // null when not loaded, empty array when loaded but no lectures
};

// 이거보다 괜찮은 네이밍 있으면 추천해주세용..
type AllTimeTableType = MyPseudoTimetable | Timetable | null;
...
export const inTimetable = (lecture: Lecture, timetable: AllTimeTableType) =>
  timetable && timetable.lectures.some((l) => l.id === lecture.id);

export const inCart = (lecture: Lecture, cart: CartType) =>
...
export const isDimmedListLectureGroup = (
...
export const getOverallLectures = (
  selectedTimetable: AllTimeTableType,
  lectureFocus: LectureFocus,
) => {
  ...
  return timetableLectures.concat(
    hasSingleFocusedLectureOutsideTable && lectureFocus.lecture ? [lectureFocus.lecture] : [],
  );
};
