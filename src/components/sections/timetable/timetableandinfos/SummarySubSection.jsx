...
class SummarySubSection extends Component {
  ...
  setFocusOnType = (type) => {
    ...
    const details = getOverallLectures(selectedTimetable, lectureFocus)
      ...
      .map((l) => ({
        ...
        info:
          l.credit > 0
            ? t('ui.others.creditCount', { count: l.credit })
            : t('ui.others.auCount', { count: l.credit_au }),
      }));
  ...
  render() {
    ...
    const totalWeight = sumBy(timetableLecturesWithReview, (l) => getWeightOfLecture(l));
                ...
                <>
                  <span
                    className={classNames(
                      'mobile-hidden',
                      isTypeCreditMultiFocused(ti) ? 'focused' : null,
                    )}>
                    {timetableTypeCredit[ti]}
                  </span>
                  <span className={classNames('mobile-hidden', 'focused')}>
                    {singleFocusedTypeCreditStr[ti]}
                  </span>
                  <span
                    className={classNames(
                      'desktop-hidden',
                      isTypeCreditMultiFocused(ti) || isTypeCreditSingleFocused(ti)
                        ? 'focused'
                        : null,
                    )}>
